...
3. **If the wikitext is template-heavy** (cppreference especially — `{{dcl}}`, `{{cpp/title}}`, etc. without template expansion look cryptic), fall back to `defuddle` on the rendered page. The rendered HTML has templates expanded:
   ...
   ```bash
   pnpx defuddle parse 'https://en.cppreference.com/w/cpp/container/vector' --markdown
   ```
...
- **Fandom: `*.fandom.com`** — **Cloudflare-protected**, plain `curl` returns the "Just a moment..." challenge. Use `fetch_content` instead. Still MediaWiki under the hood, but this reference's paths don't apply.
