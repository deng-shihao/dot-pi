...
| `npmjs.com` / `registry.npmjs.org` | `pnpm view <package> readme` for README; `curl -sL 'https://registry.npmjs.org/<package>' \| jq` for full metadata |
...
| `doi.org` / any bare DOI | `curl -sL 'https://api.crossref.org/works/<doi>' \| jq -r '.message \| .title[0], (.author[].family \| tostring)'` — reliable for title + authors + citation metadata (abstract hit-or-miss). For full text, try `curl -sL 'https://r.jina.ai/<url>'` or the publisher page |
